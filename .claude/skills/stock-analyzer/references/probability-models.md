...
> **⚠️ RETIRED FROM THE DEFAULT WORKFLOW — 2026-07-29.** Binary-catalyst betting was retired
> as the strategy on 2026-07-27 (see `decisions.md`), and stock-analyzer v2 no longer has a
> probability-model section. This file is kept for two narrow uses only:
> 1. When a dated event sits inside a momentum trade's holding window and Abbas explicitly
>    asks how bad holding through it could be (the answer is still: don't — time-stop first).
> 2. When Abbas explicitly requests a one-off binary-event assessment by name.
> The **earnings surprise**, **short squeeze**, and **M&A completion** tables below remain
> genuinely useful for momentum work; the FDA/clinical tables are legacy.

This file contains historical base rates for different catalyst types. These were the starting point for the old Section 4 probability estimates — apply company-specific modifiers on top.
